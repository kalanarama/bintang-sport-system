    ...
    <form action="{{ route('admin.promo.update', $promo->id) }}" method="POST" id="formEdit">
        ...
        <div class="row g-3 mb-3">
            <div class="col-6">
                ...
                <input type="date" name="tanggal_mulai"
                    ...
                    value="{{ old('tanggal_mulai', $promo->tanggal_mulai->format('Y-m-d')) }}">
                @error('tanggal_mulai')
                    <div class="invalid-feedback d-block">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="lapangan-checkbox-grid">
                ...
                <div class="form-check">
                    <input class="form-check-input lap-cb" type="checkbox"
                        ...
                        data-slots="{{ json_encode($promo->lapangans->firstWhere('id', $lapangan->id)?->pivot->slots ?? []) }}"
                        onchange="toggleSlot(this)"
                        {{ $promo->lapangans->contains('id', $lapangan->id) ? 'checked' : '' }}>
        ...
        <div class="mb-4">
            ...
            <select name="status_promo" class="form-select">
                <option value="1" {{ old('status_promo', $promo->status_promo ? '1' : '0') == '1' ? 'selected' : '' }}>Aktif</option>
                <option value="0" {{ old('status_promo', $promo->status_promo ? '1' : '0') == '0' ? 'selected' : '' }}>Nonaktif</option>
            </select>
        </div>
...
<script>
    ...
    function toggleSlot(cb) {
        if (cb.checked) {
            const slots = JSON.parse(cb.dataset.slots || '[]');
            renderSlot(cb.value, cb.dataset.nama, cb.dataset.harga, slots, cb.dataset.jamBuka, cb.dataset.jamTutup, cb.dataset.durasi);
        } else {
    ...
    document.getElementById('formEdit').addEventListener('submit', function(e) {
        ...
        document.getElementById('error-diskon')?.remove(); // hapus dulu error lama
